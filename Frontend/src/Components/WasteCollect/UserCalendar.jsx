...
const UserCalendar = ({ userInfo }) => { 
    ...
    const resetForm = useCallback(() => {
        setSelectedDate(null); setTimeSlot(null); setServiceArea(null);
        setEstimatedWeight(""); setPickupLocation("");
        setContactDetails({ name: userInfo?.name || "", phone: userInfo?.contactNumber || "", email: userInfo?.email || "" });
        setBookingStep(0); setBookingConfirmed(false); setBookingId("");
    ...
    const nextStep = () => { setError(null); setBookingStep(prev => prev + 1); };
    const prevStep = () => { setError(null); setBookingStep(prev => prev - 1); };

    const renderCalendar = useCallback(() => {
        const { year, month, startingDayOfWeek, daysInMonth } = getMonthData(currentDate);
        const calendarDays = [];
        for (let i = 0; i < startingDayOfWeek; i++) calendarDays.push(<div key={`empty-${i}`} className="UCal-calendar-day UCal-empty"></div>);
        ...
        switch (bookingStep) {
            ...
            case 1: return (
                <div className="UCal-step-container UCal-time-slot-selection">
                    <div className="UCal-booking-step-header"><h2>Select Pickup Time Slot</h2>{renderSummaryItem(Calendar, "Date", formatDate(selectedDate))}</div>
                    {timeSlotsForSelectedDate.length === 0 && !isLoading && <p className="UCal-info-message">No time slots available for the selected date.</p>}
                    <div className="UCal-options-grid UCal-time-slots-grid">
                        {timeSlotsForSelectedDate.map(slot => (
                            <button key={slot.id} className={`UCal-option-card UCal-time-slot-card ${timeSlot === slot.id ? 'UCal-selected' : ''}`} onClick={() => setTimeSlot(slot.id)} aria-pressed={timeSlot === slot.id}>
                                <Clock size={24} /><h3>{slot.label}</h3><p>{slot.time}</p>
                            </button>
                        ))}
                    </div>
                    <div className="UCal-step-navigation">
                        <button onClick={prevStep} className="UCal-back-button"><ChevronLeft size={16} /> Change Date</button>
                        <button onClick={nextStep} className="UCal-next-button" disabled={!timeSlot}>Next: Service Area <ChevronRight size={16} /></button>
                    </div>
                </div>
            );
            case 2: return (
